"""Multilingual critical-neuron detection sweep.

Detects per-language critical-neuron sets at one or more sparsity ratios.
For multi-ratio runs the importance cache (post gate-combine scores) is
written once per language and replayed via
``critnet.select_neurons_from_cache`` for the remaining ratios -- no
...
from critnet import CriticalNeuronConfig, NeuronDetector, select_neurons_from_cache
...
def _summary(indices: Dict[str, List[int]]) -> Dict[str, int]:
    by_module = {m: len(v) for m, v in indices.items()}
...
def _parse_top_k_percent(value: str) -> List[float]:
    ...
    for r in ratios:
        if not (0.0 < r <= 1.0):
            raise ValueError(
                f"Each top_k_percent must be in (0, 1], got {r!r} in {ratios!r}."
            )
    return ratios
...
def main() -> None:
    ...
    parser.add_argument("--stype", type=str, default="lima_s1",
                        help="Dataset type used by process_data.")
    parser.add_argument("--output_dir", type=str, default="./neuron_train_data_detect")
    parser.add_argument("--language", type=str,
                        help="Single language code (e.g., en, zh, sw)")
    parser.add_argument("--all_languages", action="store_true",
                        help="Use en, zh, ar, sw in one run")
    parser.add_argument(
        "--top_k_percent",
        type=str,
        default="0.05",
        help="Global top-k ratio, or comma-separated ratios "
             "(e.g. 0.001,0.01,0.05) to select and save for each.",
    )
    ...
    parser.add_argument("--row_modules", type=str, default=None,
                        help="Comma-separated row modules")
    parser.add_argument("--column_modules", type=str, default=None,
                        help="Comma-separated column modules")
    parser.add_argument("--norm_modules", type=str, default=None,
                        help="Comma-separated norm modules")
    parser.add_argument("--embedding_modules", type=str, default=None,
                        help="Comma-separated embedding modules")

    parser.add_argument("--include_all_eligible_modules", action="store_true",
                        help="Include attention norms (q_norm, k_norm)")
    parser.add_argument("--include_attention_norms", action="store_true",
                        help="Append q_norm and k_norm")

    parser.add_argument(
        "--save_importance_cache_dir",
        type=str,
        default=None,
        help="Directory to write per-language *.pt importance caches "
             "(post gate-combine). Reuse with --importance_cache_dir to "
             "sweep --top_k_percent without recomputing gradients.",
    )
    parser.add_argument(
        "--importance_cache_dir",
        type=str,
        default=None,
        help="Load per-language *.pt caches from this directory and only "
             "run global top-k (no model load).",
    )
    ...
    if args.importance_cache_dir and args.save_importance_cache_dir:
        raise ValueError("Use only one of --importance_cache_dir / --save_importance_cache_dir.")
    ...
    if not from_cache:
        ...
        tokenizer, _ = prepare_tokenizer(
            args.base, chat_template=tokenizer.chat_template is None,
        )
    ...
    if args.all_languages:
        ...
        target_langs = args.language.split(",")
    ...
    cache_load_root: Optional[Path] = (
        Path(args.importance_cache_dir) / args.base / args.stype
        if from_cache
        else None
    )
        ...
        for ri, ratio in enumerate(ratios):
            ...
            if from_cache:
                ...
                result = select_neurons_from_cache(
                    str(cache_pt), config, sparsity_ratio=ratio,
                )
            else:
                ...
                if ri == 0:
                    ...
                    result = NeuronDetector(model, config).detect(
                    ...
                    result = select_neurons_from_cache(
                        shared_scores_cache, config, sparsity_ratio=ratio,
                    )

            lang_dir = output_dir / lang
            config.save_pretrained(str(lang_dir), indices=result.indices)

            legacy_json = output_dir / f"{lang}.json"
            with open(legacy_json, "w", encoding="utf-8") as f:
                json.dump(result.indices, f, indent=2)

            counts = _summary(result.indices)

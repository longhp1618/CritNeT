...
# Registered LoRA name for SGLang when loading a PEFT adapter folder (must match lora_paths "name=path").
_SGLANG_PEFT_LORA_NAME = "sft"


def resolve_sglang_paths_for_eval(
    model_name: str,
    peft_base_model: str | None = None,
) -> dict:
    """Return paths for SGLang Engine, tokenizer, and config.

    If ``model_name`` is a local directory containing ``adapter_config.json`` (PEFT LoRA),
    SGLang loads ``base_model_name_or_path`` as the backbone and the adapter via ``lora_paths``.
    """
    adapter_cfg_path = os.path.join(model_name, "adapter_config.json")
    if not os.path.isfile(adapter_cfg_path):
        return {
            "engine_model_path": model_name,
            "lora_paths": None,
            "lora_name_for_generate": None,
            "config_path": model_name,
            "tokenizer_path": model_name,
        }
    with open(adapter_cfg_path, encoding="utf-8") as f:
        adapter_cfg = json.load(f)
    base = peft_base_model or adapter_cfg.get("base_model_name_or_path") or adapter_cfg.get(
        "base_model_name"
    )
    if not base:
        raise ValueError(
            f"PEFT adapter at {model_name} has no base_model_name_or_path in adapter_config.json; "
            "pass --peft_base_model."
        )
    abs_adapter = os.path.abspath(model_name)
    return {
        "engine_model_path": base,
        "lora_paths": [f"{_SGLANG_PEFT_LORA_NAME}={abs_adapter}"],
        "lora_name_for_generate": _SGLANG_PEFT_LORA_NAME,
        "config_path": base,
        "tokenizer_path": model_name,
    }
...
# Benchmark-specific configs (only intersection langs)
mmlu_langs = INTERSECTION_LANGS  # CohereLabs/Global-MMLU-Lite
...
def inference(
    ...
    gen_kwargs = {}
    if sglang_lora_name is not None:
        gen_kwargs["lora_path"] = [sglang_lora_name] * len(prompts)
    ...
    if len(modified_questions) > 0:
        ...
        gen2 = {}
        if sglang_lora_name is not None:
            gen2["lora_path"] = [sglang_lora_name] * len(modified_questions)
        new_outputs = llm.generate(modified_questions, sampling_params2, **gen2)
...
if __name__ == "__main__":
    # Example: CUDA_VISIBLE_DEVICES=4,5,6,7
    # python -m lima_s1_exp.eval.eval_utility --model_name meta-llama/Llama-3.1-8B-Instruct --dp_size 4  # add --zero_shot for zeroshot CoT
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--model_name",
        type=str,
        default="meta-llama/Llama-3.2-1B",
        help="HF model id, merged checkpoint dir, or PEFT LoRA adapter dir (with adapter_config.json).",
    )
    parser.add_argument(
        "--peft_base_model",
        type=str,
        default=None,
        help="Override backbone when --model_name is a PEFT adapter (else read base_model_name_or_path from adapter_config.json).",
    )
    parser.add_argument(
        "--zero_shot",
        action="store_true",
    ...
    paths = resolve_sglang_paths_for_eval(model_name, peft_base_model=args.peft_base_model)
    if paths["lora_paths"] is not None:
        print(
            f"PEFT LoRA eval: backbone={paths['engine_model_path']} "
            f"adapter={os.path.abspath(model_name)} "
            f"sglang_lora_name={paths['lora_name_for_generate']}"
        )
    if args.max_context_length is None:
        config = AutoConfig.from_pretrained(paths["config_path"])
    ...
    tasks = ["math_easy", "mmlu", "belebele", "arc_easy"]
    # tasks = ["math", "math_easy", "mmlu", "belebele", "gpqa", "arc_challenge", "truthfulqa", 'arc_easy']
    ...
    if args.retry_on_timeout and not args.no_retry_wrapper:
        ...
        for attempt in range(max_retries):
            proc = subprocess.run(
                [sys.executable, "-m", "lima_s1_exp.eval.eval_utility"] + retry_args,
                env=os.environ.copy(),
            )
    ...
    try:
        engine_kw: dict = {
            "model_path": paths["engine_model_path"],
            "tp_size": 1,
            "dp_size": args.dp_size,
        }
        if paths["lora_paths"] is not None:
            engine_kw["lora_paths"] = paths["lora_paths"]
    ...
    for task in tasks:
        ...
        for lang in langs:
            ...
            preds = inference(
                ...
                tokenizer=tokenizer,
                max_context_length=args.max_context_length,
                sglang_lora_name=paths["lora_name_for_generate"],
            )
            df["prediction"] = preds
